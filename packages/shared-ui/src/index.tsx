...
export function Root({ apiBaseUrl, scale, deviceId = 'dev-device' }: { apiBaseUrl: string; scale: number; deviceId?: string }) {
  ...
  const [session, setSession] = useState<{ code: string; phone: string | null } | null>(null)
  ...
  useEffect(() => { Promise.all([api<Catalog>('/catalog'), api<LearnerDto>('/me')]).then(([c, l]) => { setCatalog(c); setLearner({ ...defaultLearner, ...l }); setOffline(false) }).catch(() => setOffline(true)) }, [api])
  ...
  const save = async (highlightId: string) => {
    const r = await api<{ limit: boolean; saved: { highlight: HighlightDto } | null }>('/me/words', { method: 'POST', body: JSON.stringify({ highlightId, sessionCode: session?.code }) })
  ...
  switch (route.name) {
    case 'player': return clip ? <Player clip={clip} learner={learner} scale={scale} challenge={route.challenge && learner.plus} sessionCode={session?.code} onSave={save} onBack={() => setRoute({ name: 'home' })} onEnd={() => setRoute({ name: 'summary' })} /> : <Screen rail={rail}><T variant="body">…</T></Screen>
    ...
    case 'pair': return <Screen rail={rail}><Pair code={session?.code ?? '······'} connected={session?.phone ?? null} onLater={() => setRoute({ name: 'home' })} /></Screen>
...
// Session creation (POST /sessions) and Socket.IO join happen in the platform entry (apps/expo, apps/vega) — LING-004.
void setSessionNoop
